...
export class JobManager {
  ...
  createJob(params: { repoPath: string; repoName?: string }): AnalyzeJob {
    // Dedup: return existing active job for the same repo path
    for (const job of this.jobs.values()) {
      if (!this.isTerminal(job.status)) {
        if (params.repoPath && job.repoPath === params.repoPath) {
          return job;
        }
      }
    }
  ...
  updateJob(
    id: string,
    update: Partial<
      Pick<AnalyzeJob, 'status' | 'progress' | 'error' | 'repoName' | 'completedAt'>
    >,
  ) {
    ...
    // Emit exactly one event per updateJob call
